document.addEventListener('DOMContentLoaded', () => {
    console.log("Ad Type Overlap Script Loaded");
    ...
    if (btnNew && btnRepeat) {
        btnNew.addEventListener('click', () => {
            ...
            console.log("Ad Type: Showing New Customers");
        });

        btnRepeat.addEventListener('click', () => {
            btnRepeat.classList.add('active');
            btnNew.classList.remove('active');
            console.log("Ad Type: Showing Repeat Customers");
        });
    }


    // ==========================================
    // 3. VENN DIAGRAM TOOLTIP LOGIC
    // ==========================================
    const overlapTrigger = document.getElementById('overlapTrigger');
    const vennTooltip = document.getElementById('vennTooltip');

    if (overlapTrigger && vennTooltip) {
        // Show on Mouse Over
        overlapTrigger.addEventListener('mouseenter', () => {
            vennTooltip.classList.add('active');
        });

        // Hide on Mouse Leave
        overlapTrigger.addEventListener('mouseleave', () => {
            vennTooltip.classList.remove('active');
        });

        // Also toggle on click for mobile/touch
        overlapTrigger.addEventListener('click', (e) => {
            e.stopPropagation();
            vennTooltip.classList.toggle('active');
        });
